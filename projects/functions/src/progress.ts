...
import { Curso, LiveCourse, User } from 'shared';

const db = admin.firestore();
const courseRatingsCollection: string = "cursosValoraciones"
const courseLiveRatingsCollection: string = "cursosLiveValoraciones"
...
export const valorarCursoLive = functions.https.onCall(async (data, context) => {
  const userId = data.userId;
  const courseId = data.courseId;
  const valoracion = data.valoracion;
  ...
  if (courseId && userId && valoracion) {
    // Reconstruct DocumentReference from IDs
    const userRef = db.collection(User.collection).doc(userId);
    const courseRef = db.collection(LiveCourse.collection).doc(courseId);

    // Guardar en la coleccion de valoraciones
    const ref = db.collection(courseLiveRatingsCollection).doc();
    await ref.set(
      {
        valoracion: valoracion,
        userRef: userRef,
        liveCourseRef: courseRef,
        id: ref.id
      }, { merge: true }
    ).catch((error) => {
      return console.log(error);
    ...
    const indiceSnapshot = await db.collection(courseLiveRatingsCollection).where("liveCourseRef", "==", courseRef).get();
    const valoraciones: any[] = [];
    indiceSnapshot.forEach(doc => {
        valoraciones.push(doc.data());
    });
    ...
    console.log("Promedio", promedio);
    await db.collection(LiveCourse.collection).doc(courseId).update({
        reviewsScore: promedio,
        reviewsQty: valoraciones.length
    });
