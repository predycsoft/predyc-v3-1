...
async function updateDataEnterpriseUsageLocal(enterpriseId:string) {
  try{
    ...
    return await db.collection(Enterprise.collection).doc(enterpriseId).update({
...
export const updateDataEnterpriseUsage = functions.https.onCall(async (data, _) => {
    try {
      ...
      await updateDataEnterpriseUsageLocal(enterpriseId)
    } catch (error: any) {
      console.log(error);
