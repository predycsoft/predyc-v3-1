...
export class AdminPresentationFormComponent {
	...
	@Output() onAdminPresentationChange: EventEmitter<{ formValue: FormGroup; isEditing: boolean }> = new EventEmitter<{
		formValue: FormGroup;
		isEditing: boolean;
	}>();
	...
	async saveAdminPhoto() {
		if (this.uploadedImage) {
			if (this.adminUser.photoUrl) {
				...
				await firstValueFrom(this.storage.refFromURL(this.adminUser.photoUrl).delete()).catch((error) =>
					console.log(error)
				);
