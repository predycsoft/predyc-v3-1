...
@Component({
  ...
  styleUrls: ['./revistas-list.component.css']
})
export class RevistasListComponent {
  ...
  ngOnChanges() {
    if (this.revistas) this.performSearch(this.revistas, this.queryParamsPage,this.queryParamsSearch);
  }
  ...
  ngAfterViewInit() {
    this.queryParamsSubscription = this.activatedRoute.queryParams.subscribe((params) => {
      ...
      this.queryParamsSearch = (params["search"]) || null;
      if (this.revistas) this.performSearch(this.revistas, this.queryParamsPage,this.queryParamsSearch);
  ...
  performSearch(revistas: AuthorWithArticleQty[], page: number,search:string) {
    this.totalLength = revistas.length;
    const startIndex = (page - 1) * this.pageSize;
    const endIndex = startIndex + this.pageSize;

    revistas = revistas.filter(x => {
      if (!this.queryParamsSearch || this.queryParamsSearch === '') return true;
      return this.removeAccents(x.name.toLocaleLowerCase()).includes(this.removeAccents(this.queryParamsSearch.toLocaleLowerCase()));
    });

    this.dataSource.data = revistas.slice(startIndex, endIndex);
