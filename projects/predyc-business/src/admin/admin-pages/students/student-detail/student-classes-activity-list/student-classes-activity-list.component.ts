...
import {
  ClassByStudent,
  ClassByStudentJson,
} from "projects/shared/models/class-by-student.model";
...
export class StudentClassesActivityListComponent {
  ...
  async getTableData() {
    const dataToShowPromise: any = this.classesByStudent.map(
      async (classByStudent: ClassByStudent) => {
        const classInfo = await this.courseService.getClass(
          classByStudent.classRef.id
        );
        const courseByStudentInfo = this.coursesByStudent.find(
          (x) => x.id === classByStudent.coursesByStudentRef.id
        );
        const courseInfo = await this.courseService.getCourseById(
          courseByStudentInfo.courseRef.id
        );

        const classDateStart = firestoreTimestampToNumberTimestamp(
          classByStudent.dateStart
        );
        const classDateEnd = firestoreTimestampToNumberTimestamp(
          classByStudent.dateEnd
        );
        ...
        return {
          ...
          durationMinutes: classByStudent.completed
            ? (classDateEnd - classDateStart) / (1000 * 60)
            : null,
        };
      }
    );
