...
export class CreateDemoComponent {
  ...
  async onSubmit() {
    ...
    try {
      ...
      const enterpriseName: string =
        this.createDemoForm.controls.enterpriseName.value.trim().toLowerCase();
      const existingEnterprise = await firstValueFrom(
        this.afs
          .collection<Enterprise>(Enterprise.collection, (ref) =>
            ref.where("name", "==", enterpriseName)
          )
          .get()
      );
      if (!existingEnterprise.empty)
        throw Error("Ya existe una empresa con este nombre");

      const adminEmail: string = this.createDemoForm.controls.email.value
        .trim()
        .toLowerCase();
      const existingUser = await firstValueFrom(
        this.afs
          .collection<User>(User.collection, (ref) =>
            ref.where("email", "==", adminEmail)
          )
          .get()
      );
      if (!existingUser.empty)
        throw Error("Ya existe un usuario con este correo");
      ...
      this.alertService.succesAlert(
        "La empresa demo se ha creado correctamente"
      );
      //cerrar swal loading
    } catch (error) {
      console.log(error);
  ...
  async createLicense(enterpriseId: string) {
    const enterpriseRef: DocumentReference<Enterprise> =
      this.enterpriseService.getEnterpriseRefById(enterpriseId);
    const endDateTimestamp = dateFromCalendarToTimestamp(
      this.createDemoForm.controls.endDate.value
    );
    ...
    const productSnapshot = await firstValueFrom(
      this.afs
        .collection<Product>(Product.collection, (ref) =>
          ref.where("type", "==", Product.TYPE_FULL)
        )
        .get()
    );
  ...
  async createUsers(enterpriseId: string) {
    ...
    for (
      let i = 0;
      i < this.createDemoForm.controls.activeUsersQty.value;
      i++
    ) {
      ...
      users.push({
        ...
        displayName: `${name}`,
        email: `user${i + 1}@${this.createDemoForm.controls.enterpriseName.value
          .replace(/\s/g, "")
          .toLowerCase()}.com`,
    ...
    for (let i = 0; i < this.botsQty; i++) {
      const randomProfile =
        this.profiles[Math.floor(Math.random() * this.profiles.length)];
      let name = this.generateRandomFullName().toLowerCase();
      users.push({
        ...baseUser,
        ...
        email: `inactiveUser${
          i + 1
        }@${this.createDemoForm.controls.enterpriseName.value
          .replace(/\s/g, "")
          .toLowerCase()}.com`,
    ...
    for (let user of users) {
      ...
      let hoursUser = this.calculateVariation(
        this.randomIntFromInterval(
          Math.round(horasOptimasPlan * 0.6),
          horasOptimasPlan
        ),
        5
      );
      ...
      if (user.profile) {
        const coursesRefs: DocumentReference<Curso>[] = this.profiles
        ...
        let coursesRefUser = await this.createStudyPlan(
          newUser.uid,
          coursesRefs
        ); // aqui crea todos los datos del plan de estudio del usuario
        ...
        for (let courseRef of coursesRefs) {
          ...
          if (course && minutesUser > 0) {
            let coursesByStudent = coursesRefUser.find(
              (x) => x.courseRef.id == courseRef.id
            );
            ...
            for (let clase of classes) {
              ...
              if (minutosMonthAux <= 0) {
                let nextFecha =
                  this.obtenerPrimerDiaDelSiguienteMes(fecha).getTime();
                if (nextFecha <= today) {
                  fecha = nextFecha;
            ...
            if (progreso < 90) {
              await this.updateProgressCourse(
                coursesByStudent,
                progreso,
                progressTime,
                courseTime
  ...
  async courseStartDate(coursesByStudent, date) {
    ...
    await this.afs.collection("coursesByStudent").doc(idCourseStudent).update({
      dateStart: date,
    });
  }
  async enrollClassUser(
    userRef,
    clase,
    coursesByStudent,
    date,
    ...
    const coursesByStudentRef = this.afs
      .collection("coursesByStudent")
      .doc(coursesByStudent.id).ref;
    ...
    try {
      const ref = this.afs
        .collection<StudyPlanClass>(StudyPlanClass.collection)
        .doc().ref;
      await ref.set(
        { ...classStudyPlan.toJson(), id: ref.id },
        { merge: true }
      );
  ...
  async createStudyPlan(
    studentUid: string,
    coursesRefs: DocumentReference<Curso>[]
    ...
    for (let i = 0; i < coursesRefs.length; i++) {
      const userRef: DocumentReference<User> =
        this.userService.getUserRefById(studentUid);
      const courseData = this.courses.find(
        (courseData) => courseData.id === coursesRefs[i].id
      );
      const courseDuration = courseData.duracion;
      endDate = this.courseService.calculatEndDatePlan(
        startDate,
        courseDuration,
        this.hoursPerMonthForStudyPlan
      );
      const enrolledCourse = await this.courseService.saveCourseByStudent(
        coursesRefs[i],
        userRef,
        new Date(startDate),
        new Date(endDate),
