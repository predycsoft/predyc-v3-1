...
export class ActivityClassesService {
  constructor(private afAuth: AngularFireAuth, private afs: AngularFirestore, private enterpriseService: EnterpriseService, private alertService: AlertsService, private courseService: CourseService, private liveCourseService: LiveCourseService) {
  ...
  async addQuestion(activityId: string, newQuestion: Question): Promise<void> {
    const ref = this.afs.collection<Question>(Activity.collection).doc(activityId).collection(Question.collection).doc().ref;
  ...
  async saveQuestion(newQuestion: Question, idActivity): Promise<void> {
    ...
    try {
      ...
      if (!newQuestion.id) {
        ...
        ref = this.afs.collection(Activity.collection).doc(idActivity).collection(Question.collection).doc().ref;
        newQuestion.id = ref.id;
      } else {
        // If an ID exists, just reference the existing document
        // console.log('actualizar pregunta')
        ref = this.afs.collection(Activity.collection).doc(idActivity).collection(Question.collection).doc(newQuestion.id).ref;
      }
  ...
  async removeQuestions(QuestionsIds: string[], idActivity: string): Promise<void> {
    try {
      ...
      const questionsCollectionRef = this.afs.collection(Activity.collection).doc(idActivity).collection(Question.collection);
  ...
  getQuestionsCourses(courseIds: string[]): Observable<any> {
    ...
    return this.afs
      .collection("activity", (ref) => ref.where("coursesRef", "array-contains-any", courseRefs))
      .get()
      .pipe(
  ...
  getActivityAndQuestionsForCourse(courseId: string, isLiveCourse: boolean): Observable<any[]> {
    ...
    return this.afs
      .collection("activity", (ref) => ref.where("coursesRef", "array-contains", courseRef).where("type", "==", "regular"))
      .get()
      .pipe(
        switchMap((activitiesSnap) => {
  ...
  getActivityProfile(idProfile) {
    ...
    return this.afs
      .collection("activity", (ref) => ref.where("profileRef", "==", profileRef))
  ...
  getActivityCourse(idCourse: string, type: "course" | "liveCourse" | "liveCourseTemplate") {
    let courseRef: DocumentReference;
    ...
    // console.log('courseRef getActivityCourse',courseRef)
    return this.afs
      .collection("activity", (ref) => ref.where("coursesRef", "array-contains", courseRef).where("type", "==", "test"))
      .valueChanges() // or .get() based on your needs
      .pipe(
        switchMap((activities) => {
          if (activities && activities.length > 0) {
  ...
  getActivityById(idActivity: string): Observable<any> {
    ...
    return activityDocRef.snapshotChanges().pipe(
      switchMap((doc) => {
        ...
        return this.afs
          .collection<Question>(`${Activity.collection}/${idActivity}/${Question.collection}`)
          .snapshotChanges()
          .pipe(
            map((actions) => {
  ...
  getActivityCertificationResultsEnterprise(): Observable<any> {
    ...
    return this.enterpriseService.enterpriseLoaded$.pipe(
      switchMap((enterpriseIsLoaded) => {
        if (enterpriseIsLoaded) {
          const enterpriseRef = this.enterpriseService.getEnterpriseRef();
          return this.afs.collection<any>("profileTestsByStudent", (ref) => ref.where("enterpriseRef", "==", enterpriseRef).where("certificationTest", "==", true)).valueChanges({ idField: "id" });
        } else {
  ...
  getTestProfileResultsEnterprise(): Observable<any> {
    ...
    return this.enterpriseService.enterpriseLoaded$.pipe(
      switchMap((enterpriseIsLoaded) => {
        if (enterpriseIsLoaded) {
          const enterpriseRef = this.enterpriseService.getEnterpriseRef();
          return this.afs.collection<any>("profileTestsByStudent", (ref) => ref.where("enterpriseRef", "==", enterpriseRef)).valueChanges({ idField: "id" });
        } else {
          // Devuelve un Observable vacío que no emite valores
          return of([]);
        }
  ...
  getActivities() {
    ...
    this.activityCollectionSubscription = this.afs
      .collection<Activity>(Activity.collection, (ref) => ref.where("enterpriseRef", "==", this.enterpriseService.getEnterpriseRef()).orderBy("updatedAt", "desc"))
      .valueChanges()
  ...
  getActivityCertifications(): Observable<any> {
    return this.afs
      .collection(Activity.collection, (ref) => ref.where("type", "==", "testCertification"))
