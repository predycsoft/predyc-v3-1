...
export class LicenseService {
  ...
  getEnterpriseLicenses(): Promise<License[]> {
    return firstValueFrom(
      this.enterpriseService.enterpriseLoaded$.pipe(
        take(1),
        switchMap((isLoaded) => {
  ...
  async assignLicenseWithRotations(usersIds: string[]) {
    // migrar a cloud funtion
    try {
          ...
          licencia = licenses
            .filter(
              (license) =>
                ...
                license.rotationsWaitingCount > 0
            )
            .sort((a, b) => a.currentPeriodStart - b.currentPeriodStart);
          rotation = true;
        } else {
          licencia = licenses
            .filter(
  ...
  async assignLicense(license: License, usersIds: string[], rotation = false) {
    const licenseRef: DocumentReference<License> = this.afs
      .collection<License>(License.collection)
      .doc(license.id).ref;
    if (license.quantityUsed + usersIds.length > license.quantity) {
      this.alertService.errorAlert(
        "No tienes suficientes cupos en esta licencia para la cantidad de usuarios seleccionados"
      );
      return;
    } else {
      ...
      //if (dialogResult) {
      if (true) {
        const createSubscriptionsPromises = usersIds.map((userId) =>
          this.subscriptionService.createUserSubscriptionByLicense(
            license,
            licenseRef,
            userId
          )
        ...
        console.log(
          ...
          rotation && license.rotations > license.rotationsUsed
        );
            ...
            .set(
              {
                ...
                rotationsWaitingCount:
                  license.rotationsWaitingCount - usersIds.length,
              },
              { merge: true }
            );
        } else {
...
  }
  ...
  async removeLicense(usersIds: string[], licenses): Promise<void> {
    const dialogResult = await firstValueFrom(
      this.dialogService.dialogConfirmar().afterClosed()
    );
    if (dialogResult) {
      // let promises = []
      ...
      const promises = usersIds.map((userId) =>
        this.subscriptionService.removeUserSubscription(userId, licenses)
      );
