...
export class StudentProfileComponent implements OnInit {
	...
	ngOnInit(): void {
		...
		if (!this.student.uid) {
			...
			this.form
				.get("profile")
				?.setValue(this.student.profile && this.student.profile.id ? this.student.profile.id : null);
		}
	}
	...
	async saveStudentPhoto() {
		if (this.uploadedImage) {
			if (this.student.photoUrl) {
				...
				await firstValueFrom(this.storage.refFromURL(this.student.photoUrl).delete()).catch((error) =>
					console.log(error)
				);
