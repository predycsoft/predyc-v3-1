  ...
  ngOnInit() {
    ...
    this.enterpriseService.enterpriseLoaded$.subscribe(async isLoaded => {
      if (isLoaded) {
        ...
        const profiles = await firstValueFrom(this.profileService.getProfiles$(enterpriseRef))
        const departments = await firstValueFrom(this.departmentService.getDepartments$(enterpriseRef))
        const courses = await firstValueFrom(this.courseService.getCourses$(enterpriseRef))
