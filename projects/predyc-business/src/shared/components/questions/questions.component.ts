...
export class QuestionsComponent {
  ...
  @Input() enableImportQuestions = true;
  @Input() enableSortQuestions = true;
  ...
  uploadQuestions(evt) {
    ...
    reader.onload = async (e: any) => {
      ...
      preguntas.forEach((pregunta) => {
        ...
        const preguntaIndex = this.questions.controls.findIndex((c) => c.get("idUser").value === idUser);
        ...
        if (newType == this.QuestionClass.TYPE_TRUE_OR_FALSE) {
          ...
          opciones = pregunta.opciones
            ...
            .map((opcion) => ({
              image: null,
              isCorrect: pregunta?.respuestaCorrecta
                ?.split(",")
                .map((letra) => letra.trim())
                .includes(opcion.letra),
              placeholder: null,
              text: opcion.texto,
            }));
        }
  ...
  uniqueIdUserValidator(questionsFormArray: FormArray): ValidatorFn {
    return (control: AbstractControl): ValidationErrors | null => {
      ...
      const idUserExists = questionsFormArray.controls.some((formGroup) => formGroup.get("idUser")?.value === idUser && formGroup !== control.parent);
  ...
  addOption(questionIndex: number, placeholder = null, image = null, text = ""): void {
    this.options(questionIndex).push(
      this.fb.group({
        image: [image],
  ...
  addQuestionSkill(questionIndex: number, skill): void {
    this.questionSkills(questionIndex).push(
      this.fb.group({
        id: [skill.id],
        name: [skill.name],
        categoryId: [skill.categoryId],
  ...
  getQuestionInstruction(questionIndex: number) {
    return this.sanitizer.bypassSecurityTrustHtml(Question.typeToInfoDict[this.questions.at(questionIndex)["controls"]["type"].value].createInstructions);
  }
    ...
    for (let index = 0; index < this.options(questionIndex).controls.length; index++) {
      if (this.options(questionIndex).at(index).get("placeholder").value === targetPlaceholder) {
        this.options(questionIndex)
          .at(index)
          .get("isCorrect")
          .setValue(index === optionIndex ? true : false);
      }
    }
    //this.changeQuestion.emit(this.mainForm);
    //this.formatExamQuestions()
  ...
  showDisplayText(questionIndex) {
    this.questionStatus[questionIndex].textToRender = this.sanitizer.bypassSecurityTrustHtml(this.getDisplayText(questionIndex));
  }
  ...
  getDisplayText(questionIndex: number): string {
    ...
    for (const placeholder of placeholders) {
      const options = this.options(questionIndex)["controls"].filter((option) => option.get("placeholder").value == placeholder);
