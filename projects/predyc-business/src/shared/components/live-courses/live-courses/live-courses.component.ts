...
export class LiveCoursesComponent {
  constructor(public icon: IconService, public categoryService: CategoryService, public courseService: CourseService, public skillService: SkillService, private enterpriseService: EnterpriseService, private authService: AuthService, private productService: ProductService, public licenseService: LicenseService, public liveCourseService: LiveCourseService, public instructorService: InstructorsService, private modalService: NgbModal) {}
  ...
  async ngOnInit() {
    ...
    combineLatest([this.categoryService.getCategoriesObservable(), this.skillService.getSkills$(), this.liveCourseService.getAllLiveCoursesTemplatesWithSessionsTemplates$(), this.instructorService.getInstructors$()]).subscribe(async ([categories, skills, coursesData, instructors]) => {
      ...
      this.categories.forEach((category) => {
        let filteredCourses = courses.filter((course) => course["categories"].some((cat) => cat.id === category.id));
  ...
  getFormattedDuration(): string {
    const hours = Math.floor(this.selectedCourse.duration / 60);
    const minutes = this.selectedCourse.duration % 60;
  ...
  anidarCompetenciasInicial(categorias: any[], skills: any[]): any[] {
    return categorias.map((categoria) => {
      let filteredSkills = skills
        .filter((x) => x.category.id === categoria.id)
        .map((skill) => {
